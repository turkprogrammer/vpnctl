...
- Утилиты AmneziaWG: `awg`, `awg-quick`.
...
> только библиотеки рантайма `libgtk-3` / `libayatana-appindicator3`.
...
| `./vpnctl tray` | Открыть графический интерфейс |
...
Команда `status` и большая часть проверок работают от обычного пользователя:
...
Привилегированные операции (`up`, `down`, `import`) требуют подтверждения через
`pkexec` там, где в sudoers нет правила `NOPASSWD`. Для повторяемых тестов можно
переопределить каталог конфигов переменной `VPNCTL_CONF_DIR`, а адрес проверки
внешнего IP — переменной `VPNCTL_IP_URL`.
